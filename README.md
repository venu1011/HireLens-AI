...
- **Smart Resume–JD Diff View** — side-by-side visual comparison with color-coded highlights
- **AI Auto-Optimize** — one-click Gemini-powered resume rewrite with PDF download
- **Canonical skill matching** using 100+ skill aliases for accurate job-match analysis
- **Premium dark UI** with glass-morphism cards, Framer Motion animations, and gradient accents
...
│   │   ├── diffGenerator.js      # Resume–JD diff highlight engine
...
| Layer       | Technology                                                  |
|-------------|-------------------------------------------------------------|
| Frontend    | React 18, Vite, Tailwind CSS, Chart.js, Framer Motion, React Router v6 |
...
| PDF Parse   | pdf-parse                                                  |
| UI Extras   | react-hot-toast, react-icons (Feather Icons), Framer Motion  |
...
| **Smart Diff View**           | Side-by-side resume vs JD comparison with color-coded highlights (matched, missing, weak verbs, metrics) |
| **AI Auto-Optimize**          | One-click Gemini AI resume rewrite — stronger bullets, targeted keywords, optimized structure     |
| **PDF Export**                | Download your AI-optimized resume as a professionally formatted PDF                              |
...
| **Framer Motion Animations**  | Smooth entrance animations, staggered reveals, scroll-triggered effects across all pages         |
| **Auth-Aware UI**             | Homepage adapts CTAs based on login state — shows Dashboard/Analyze when signed in               |
...
| GET    | `/api/analysis/:id/diff`| Yes  | Get diff view data (resume + JD highlights) |
| POST   | `/api/analysis/:id/optimize` | Yes | AI auto-optimize resume for the job  |
| POST   | `/api/analysis/:id/download-pdf` | Yes | Download optimized resume as PDF  |
...
|-------------------|------------------|----------------------------------------------------------------|
| Home              | `/`              | Animated landing page with Framer Motion — adapts CTAs for logged-in users |
| Register          | `/register`      | Create account with glass-morphism form                        |
| Login             | `/login`         | Sign in with JWT token                                         |
| Dashboard         | `/dashboard`     | Animated overview cards, latest resume with grade, recent analyses |
| Analyze           | `/analyze`       | 3-step flow: select/upload resume → paste JD → confirm & run   |
| Analysis Result   | `/analysis/:id`  | 7 tabs: Overview, Diff View, Feedback, Skills, Suggestions, Optimize, Roadmap |
...
- **Animations**: Framer Motion entrance animations, staggered reveals, scroll-triggered effects, CSS shimmer
- **Components**: Custom `.card`, `.btn-primary`, `.input-field` classes
- **Charts**: Chart.js via react-chartjs-2 (Doughnut, Radar, Line)
- **PDF Gen**: PDFKit with styled sections, blue accent lines, and bullet formatting
